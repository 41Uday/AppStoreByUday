...
class AppStore extends Component {
  state = {searchInput: '', tabVal: tabsList[0].tabId}
  ...
  tabSearch = tabId => {
    console.log('Triggered', tabId)
    this.setState({tabVal: tabId})
  }

  render() {
    const {searchInput, tabVal} = this.state
    console.log(tabVal)
    const result = appsList.filter(e => e.category === tabVal)
    console.log(result)
    const filteredSearches = result.filter(e =>
    ...
    return (
      <div className="bg-container">
        ...
        <ul>
          {tabsList.map(e => (
            <TabItem key={e.tabId} tab={e} tabSearch={this.tabSearch} />
          ))}
        </ul>
        <ul>
          {filteredSearches.map(e => (
            <AppItem key={e.appId} appListItem={e} />
          ))}
        </ul>
      </div>
    )
